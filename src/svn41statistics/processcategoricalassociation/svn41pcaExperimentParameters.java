...
public class svn41pcaExperimentParameters {

	public static final Semaphore svn = new Semaphore(1);

	public static final File baseDirectory = SystemUtils.IS_OS_LINUX ? new File(
			"/home/sander/Documents/svn/41 - stochastic statistics/experiments/08 - process categorical association sensitivity")
			: new File(
					"C:\\Users\\leemans2\\Documents\\svn\\41 - stochastic statistics\\experiments\\08 - process categorical association sensitivity");
	...
	public svn41pcaExperimentParameters() {
	...
	public List<svn41pcaCall> getCalls() {
		List<svn41pcaCall> result = new ArrayList<>();
			...
			if (logFile.getName().endsWith(".xes.gz") && log2attribute.containsKey(logFile.getName())) {
				for (int numberOfSamples : getNumbersOfSamples()) {
					for (int sampleSize : getSampleSizes()) {
						result.add(new svn41pcaCall(logFile.getName(), numberOfSamples, sampleSize, this,
	...
	public File getAssociationDirectory() {
		return new File(baseDirectory, "1-association");
	}
