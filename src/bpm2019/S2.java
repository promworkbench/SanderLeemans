...
public class S2 {
	@Plugin(name = "BPM2019 S2 Create GSPN", returnLabels = { "Stochastic Petri net S2" }, returnTypes = {
	...
	public StochasticNet convert(final PluginContext context) {
		...
		TimedTransition t1 = result.addImmediateTransition("silent start", 0.5);
		...
		TimedTransition d = result.addImmediateTransition("d", 0.5);
		result.addArc(p1, d);
