...
public class CostModelComputerImplLP extends CostModelComputerAbstract {
	...
	public void compute(List<Pair<double[], Double>> data, CostModelAbstract result, IvMCanceller canceller)
		...
		//compute average absolute error
		{
			...
			result.getModelProperties().add(new DataRow<Object>("cost model (global)", "total absolute error",
					DisplayType.numeric(sum.doubleValue())));
			result.getModelProperties().add(new DataRow<Object>("cost model (global)", "average absolute error",
		...
		//set assumptions
		{
			...
			result.getModelProperties().add(new DataRow<Object>(DisplayType.literal("all parameters ≥ 0"),
					"cost model (global)", "assumption"));
